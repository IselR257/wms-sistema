...
const enrichOrdersWithReturnData = (orders) => orders.map((order) => {
    const returnedByDetail = Object.fromEntries(order.detalles.map((detail) => {
        const total = (detail.devoluciones || []).reduce((sum, item) => sum + Number(item.cantidad), 0);
    ...
    return {
        ...
        devoluciones: (order.devoluciones || []).map((devolucion) => ({
            ...devolucion,
            totalItems: devolucion.detalles.reduce((sum, item) => sum + Number(item.cantidad), 0)
        }))
    };
});
...
despachosRouter.get("/ordenes", asyncHandler(async (_req, res) => {
...
despachosRouter.post("/ordenes", requirePermission("Despachos"), asyncHandler(async (req, res) => {
...
despachosRouter.post("/:id/generar-picking", requirePermission("Picking"), asyncHandler(async (req, res) => {
...
despachosRouter.post("/:id/finalizar", requirePermission("Despachos", "Picking"), asyncHandler(async (req, res) => {
    const ordenId = String(req.params.id);
...
despachosRouter.post("/:id/devolucion", requirePermission("Despachos"), asyncHandler(async (req, res) => {
    const ordenId = String(req.params.id);
