...
reportesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const movementByDayMap = new Map<string, { fecha: string; entradas: number; salidas: number; ajustes: number }>();
    const rotationMap = new Map<
      string,
      { productoId: string; producto: string; categoria: string; entradas: number; salidas: number; ajustes: number; total: number }
    >();
    ...
    for (const movimiento of movimientos) {
      ...
      if (movimiento.tipo === "ENTRADA") day.entradas += cantidad;
      if (movimiento.tipo === "SALIDA") day.salidas += cantidad;
      if (movimiento.tipo === "AJUSTE") day.ajustes += cantidad;
      ...
      if (movimiento.tipo === "ENTRADA") rank.entradas += cantidad;
      if (movimiento.tipo === "SALIDA") rank.salidas += cantidad;
      if (movimiento.tipo === "AJUSTE") rank.ajustes += cantidad;
      rank.total += cantidad;
    ...
    const dispatchUnits = despachos.reduce(
      (sum, order) => sum + order.detalles.reduce((detailSum, detail) => detailSum + Number(detail.cantidad), 0),
      0
    );
    const purchaseUnits = compras.reduce(
      (sum, order) => sum + order.detalles.reduce((detailSum, detail) => detailSum + Number(detail.cantidad), 0),
      0
    );
